...
import { initSatellite, onAuthStateChange, listDocs, uploadFile } from "@junobuild/core";
...
export default function AdminBusinessMessagingPage() {
  ...
  const [businesses, setBusinesses] = useState<ApplicationData[]>([]);
  ...
  const loadBusinesses = async () => {
    setLoading(true);
    try {
      const result = await listDocs<ApplicationData>({ collection: "business_applications" });
      setBusinesses(result.items.map(doc => doc.data).filter(b => b.status === "approved"));
    } catch (error) {
    ...
    try {
      if (bulkMode) {
        ...
        const result = await sendPlatformMessage(
          {
            to: businessId,
            toName: businesses.find(b => b.businessName === selectedBusiness)?.businessName,
            subject,
            content,
            type: messageType,
            attachments,
  ...
  const toggleBusinessSelection = (businessName: string) => {
    ...
    if (newSelection.has(businessName)) {
      newSelection.delete(businessName);
    } else {
      newSelection.add(businessName);
    }
    setSelectedBusinesses(newSelection);
  };

  const selectAll = () => {
    setSelectedBusinesses(new Set(businesses.map(b => b.businessName)));
  };
        ...
        <div className="bg-white dark:bg-neutral-900 rounded-2xl border-2 border-neutral-200 dark:border-neutral-800 p-6 max-w-4xl mx-auto">
            ...
            <div className="mb-6">
              ...
              >
                ...
                {businesses.map((business, idx) => (
                  <option key={idx} value={business.businessName}>
              ...
              <div className="max-h-60 overflow-y-auto border border-neutral-300 dark:border-neutral-700 rounded-lg p-3 space-y-2">
                {businesses.map((business, idx) => (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={selectedBusinesses.has(business.businessName)}
                      onChange={() => toggleBusinessSelection(business.businessName)}
                      className="w-4 h-4 text-primary-600 rounded focus:ring-2 focus:ring-primary-500"
                    />
                    <div className="flex-1">
                      <p className="text-sm font-medium text-neutral-900 dark:text-white">{business.businessName}</p>
                      <p className="text-xs text-neutral-600 dark:text-neutral-400">{business.industry}</p>
                    </div>
                  </label>
                ))}
              </div>
          ...
          <div className="mb-6">
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-white dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 rounded-lg text-neutral-900 dark:text-white placeholder-neutral-500 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
            />
          </div>
          ...
          <button
            ...
            disabled={sending || !subject.trim() || !content.trim() || (!bulkMode && !selectedBusiness) || (bulkMode && selectedBusinesses.size === 0)}
