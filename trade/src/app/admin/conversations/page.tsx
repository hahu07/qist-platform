...
export default function AdminConversationsPage() {
  ...
  const loadThreads = async () => {
    if (!user) return;
    try {
      const conversationThreads = await getConversationThreads(user.key, true);
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      // Determine recipient businessId from thread
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-bold text-neutral-900 dark:text-white">Business Conversations</h1>
            </div>
          ...
          <div className="lg:col-span-1 bg-white dark:bg-neutral-900 rounded-lg border border-neutral-200 dark:border-neutral-800 shadow-sm">
            ...
            <div className="divide-y divide-neutral-200 dark:divide-neutral-700 max-h-[600px] overflow-y-auto">
              {threads.length === 0 ? (
                ...
                threads.map((thread) => {
                  const businessName = thread.thread.data.businessName || 
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs text-neutral-500 dark:text-neutral-500">
                          {thread.lastMessage.data.from === "platform" ? "Admin" : "Business"}
                        </span>
                      </div>
                    </button>
                  );
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-primary-600 dark:text-primary-400">
                        {selectedThread.thread.data.businessName || "Business Conversation"}
                      </p>
                    </div>
                        ...
                        <div className={`max-w-[80%] ${isFromAdmin ? "bg-primary-100 dark:bg-primary-900/30" : "bg-neutral-100 dark:bg-neutral-800"} rounded-lg p-4`}>
                          <div className="flex items-center gap-2 mb-2">
                            <span className={`text-xs font-semibold ${isFromAdmin ? "text-primary-700 dark:text-primary-300" : "text-neutral-700 dark:text-neutral-300"}`}>
                              {isFromAdmin ? "You (Admin)" : (msg.data.fromName || "Business")}
                            </span>
                            <span className="text-xs text-neutral-500 dark:text-neutral-500">
