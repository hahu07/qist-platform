...
import { ContractTermsReview } from "@/components/admin/contract-terms-review";
...
function AdminApplicationsPageContent() {
  ...
  const [activeTab, setActiveTab] = useState<'details' | 'due-diligence' | 'pricing' | 'contract-terms' | 'recommendations'>('details');
  ...
  const handleContractTermsCounterOffer = async (application: Application, counterTerms: any) => {
    try {
      setProcessing(true);
      ...
      // Update application with counter-offer terms
      await setDoc({
        collection: "business_applications",
        doc: {
          ...latestDoc,
          data: {
            ...latestDoc.data,
            contractTermsCounterOffer: counterTerms,
            contractTermsStatus: 'counter-offered',
  ...
  const handleContractTermsApprove = async (application: Application) => {
    try {
      setProcessing(true);
      ...
      // Lock the contract terms as final
      await setDoc({
        collection: "business_applications",
        doc: {
          ...latestDoc,
          data: {
            ...
            finalContractTerms: latestDoc.data.contractTerms,
            updatedAt: new Date().toISOString()
          }
        }
      });

      toast.success('Contract terms approved successfully');
      setSelectedApplication(null);
      await fetchApplications();
    } catch (error) {
      console.error('Failed to approve contract terms:', error);
      toast.error('Failed to approve contract terms. Please try again.');
    } finally {
      setProcessing(false);
    }
  };

  const handleContractTermsRequestRevision = async (application: Application, message: string) => {
    try {
      setProcessing(true);
      ...
      await setDoc({
        ...
        doc: {
          ...
          data: {
            ...latestDoc.data,
            contractTermsStatus: 'revision-requested',
            contractTermsRevisionMessage: message,
            updatedAt: new Date().toISOString()
          }
        }
      });
              ...
              <div className="px-8 py-6 max-h-[60vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                  ...
                  <ContractTermsReview 
                    ...
                    onCounterOffer={(terms) => handleContractTermsCounterOffer(selectedApplication, terms)}
                    onApprove={() => handleContractTermsApprove(selectedApplication)}
                    onRequestRevision={(message) => handleContractTermsRequestRevision(selectedApplication, message)}
                  />
                ) : activeTab === 'recommendations' ? (
              ...
              <div className="px-8 py-6 border-t border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900/50">
                {!showApprovalForm && !showRequestInfoForm ? (
                  <div className="flex flex-wrap gap-3">
                    <button
                      onClick={() => {
                        // Auto-calculate investment terms from contract terms
                        const contractTerms = selectedApplication.data.contractTerms as ContractTerms | undefined;
                        const calculatedTerms = calculateInvestmentTermsFromContract(contractTerms);
                        
                        if (calculatedTerms) {
                          // Use calculated terms
                          setApprovalTerms(calculatedTerms);
                        } else {
                          // Fallback: Set default terms based on contract type
                  ...
                  <div className="space-y-4">
                    <div className="flex items-start justify-between">
                      <div>
                        <h3 className="font-bold text-lg text-neutral-900 dark:text-white">Investment Terms</h3>
                        {(() => {
                          const contractTerms = selectedApplication.data.contractTerms as ContractTerms | undefined;
                            ...
                            return (
                              <div className="mt-2 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                                <div className="flex items-start gap-2">
                                  ...
                                  <div>
                                    <p className="text-sm font-medium text-blue-900 dark:text-blue-100 mb-1">Auto-calculated from Contract Terms</p>
                          ...
                          return (
                            <p className="mt-1 text-sm text-neutral-600 dark:text-neutral-400">Review and adjust terms as needed</p>
                          );
                        })()}
                      </div>
                    </div>
