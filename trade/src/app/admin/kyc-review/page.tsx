...
import { kycRejectionReasons } from "@/schemas";
...
export default function KYCReviewPage() {
  ...
  // New state for rejection and request info dialogs
  const [showRejectionDialog, setShowRejectionDialog] = useState(false);
  const [investorToReject, setInvestorToReject] = useState<Investor | null>(null);
  ...
  const handleApproveKYC = async (investor: Investor) => {
    try {
      ...
      if (isPending) {
        ...
        await setDoc({
          ...
          doc: {
            ...
            data: {
              ...investor.data,
              memberNumber: investor.data.memberNumber || memberNumber, // Assign if not already assigned
              kycStatus: 'verified'
            },
      ...
      const { notifyKYCApproved } = await import('@/utils/notification-actions');
      const memberName = getInvestorName(investor);
      
      await notifyKYCApproved(investor.key, memberName, memberNumber);
      
      // Send SMS/WhatsApp notification if phone available and enabled
      const phoneNumber = (investor.data as any).phone;
      const smsEnabled = (investor.data as any).smsNotifications !== false; // Default to true
      ...
      if (phoneNumber && smsEnabled) {
        ...
        try {
          await sendKYCApprovedNotification({
            ...
            memberNumber,
            sendSMS: true,
            sendWhatsApp: true, // Send both by default
          });
        } catch (smsError) {
          console.error('SMS/WhatsApp notification failed:', smsError);
          // Don't fail the whole approval if SMS fails
        }
      }
  ...
  const handleRejectKYC = async () => {
    ...
    try {
      setProcessing(true);
      const collection = investorToReject.data.investorType === 'individual' 
        ? "individual_investor_profiles" 
        : "corporate_investor_profiles";
      
      // Find the label for the selected rejection reason
      const allReasons = [...kycRejectionReasons.resubmittable, ...kycRejectionReasons.permanent];
      const reasonLabel = allReasons.find((r: { value: string; label: string }) => r.value === rejectionReason)?.label || rejectionReason;
        
      await setDoc({
        collection,
        doc: {
          key: investorToReject.key,
          data: {
            ...investorToReject.data,
            kycStatus: 'rejected',
            ...
            rejectedAt: new Date().toISOString(),
            rejectedBy: user?.key || 'admin'
          },
          version: investorToReject.version
        }
      });
      
      // Send in-app notification to member
      const { notifyKYCRejected } = await import('@/utils/notification-actions');
      const memberName = getInvestorName(investorToReject);
      const message = customRejectionMessage.trim() || reasonLabel;
      ...
      // Send SMS/WhatsApp notification if phone available and enabled
      const phoneNumber = (investorToReject.data as any).phone;
      const smsEnabled = (investorToReject.data as any).smsNotifications !== false; // Default to true
      ...
      if (phoneNumber && smsEnabled) {
        ...
        try {
          ...
          console.error('SMS/WhatsApp notification failed:', smsError);
          // Don't fail the whole rejection if SMS fails
        }
      }
      ...
      alert(`KYC rejected for ${getInvestorName(investorToReject)}`);
    } catch (error) {
  ...
  const handleRequestInfo = async () => {
    ...
    try {
      setProcessing(true);
      const collection = selectedInvestor.data.investorType === 'individual' 
        ? "individual_investor_profiles" 
        : "corporate_investor_profiles";
      ...
      await setDoc({
        collection,
        doc: {
          key: selectedInvestor.key,
          data: {
            ...selectedInvestor.data,
            kycStatus: 'pending',
            adminMessage: requestInfoMessage.trim(),
            requestedAt: new Date().toISOString()
          },
          version: selectedInvestor.version
        }
      });
      ...
      alert(`More information requested from ${getInvestorName(selectedInvestor)}`);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-neutral-50 via-white to-primary-50/30 dark:from-neutral-950 dark:via-neutral-900 dark:to-neutral-950">
                ...
                {selectedInvestor.data.kycStatus !== 'verified' && (
                  <>
                    <button
                      ...
                      disabled={processing}
                      className="flex-1 px-4 py-3 bg-success-600 hover:bg-success-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg font-medium transition-colors"
                    >
                      ✓ Approve KYC
                    </button>
                    {selectedInvestor.data.kycStatus !== 'rejected' && (
                      <button
                        onClick={() => {
                          setInvestorToReject(selectedInvestor);
                          setShowRejectionDialog(true);
                        }}
                        disabled={processing}
                        className="flex-1 px-4 py-3 bg-error-600 hover:bg-error-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg font-medium transition-colors"
                      >
                        ✗ Reject KYC
                      </button>
                    )}
                    <button
                      ...
                      disabled={processing}
                      className="flex-1 px-4 py-3 bg-warning-600 hover:bg-warning-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg font-medium transition-colors"
                    >
                      ℹ Request Info
                    </button>
                  </>
      ...
      {/* KYC Rejection Dialog Modal */}
      {showRejectionDialog && investorToReject && (
          ...
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div className="relative bg-white dark:bg-neutral-900 rounded-2xl max-w-3xl w-full shadow-2xl border border-neutral-200 dark:border-neutral-800">
              ...
              <div className="px-6 py-4 border-b border-neutral-200 dark:border-neutral-800">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-xl font-bold text-error-600 dark:text-error-400">Reject KYC Application</h3>
                    <p className="text-sm text-neutral-600 dark:text-neutral-400 mt-1">
                      {getInvestorName(investorToReject)}
                    </p>
                  </div>
                  <button 
                    onClick={() => {
              ...
              <div className="px-6 py-4 space-y-6">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-neutral-600 dark:text-neutral-400 mt-1">
                            Member can address concerns and resubmit
                          </div>
                        </div>
                      </div>
                    </button>
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-neutral-600 dark:text-neutral-400 mt-1">
                            Member cannot resubmit this application
                          </div>
                        </div>
                      </div>
                    </button>
                  ...
                  {rejectionAllowsResubmit ? (
                    <div className="space-y-2">
                      <p className="text-xs text-neutral-600 dark:text-neutral-400 mb-2">
                        Select a fixable issue that the member can address:
                      </p>
                      {kycRejectionReasons.resubmittable.map((reason: { value: string; label: string }) => (
                        <label
                          key={reason.value}
                    ...
                    <div className="space-y-2">
                      ...
                      {kycRejectionReasons.permanent.map((reason: { value: string; label: string }) => (
                        <label
                          key={reason.value}
                          className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-all ${
                            rejectionReason === reason.value
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Provide additional details or suggestions for the member..."
                    rows={4}
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-error-700 dark:text-error-400 mt-1">
                          This member will not be able to resubmit their KYC application. They will need to contact support for assistance. Use this only for serious eligibility issues.
                        </p>
                      </div>
                    </div>
                  </div>
      ...
      {/* Request More Information Dialog Modal */}
      {showRequestInfoDialog && selectedInvestor && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            <div className="fixed inset-0 bg-black/50 backdrop-blur-sm transition-opacity" onClick={() => setShowRequestInfoDialog(false)} />
            ...
            <div className="relative bg-white dark:bg-neutral-900 rounded-2xl max-w-2xl w-full shadow-2xl border border-neutral-200 dark:border-neutral-800">
              ...
              <div className="px-6 py-4 border-b border-neutral-200 dark:border-neutral-800">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-xl font-bold text-warning-600 dark:text-warning-400">Request More Information</h3>
                    <p className="text-sm text-neutral-600 dark:text-neutral-400 mt-1">
              ...
              <div className="px-6 py-4 space-y-6">
                <div>
                  <label className="block text-sm font-semibold text-neutral-900 dark:text-white mb-3">
                    Message to Member <span className="text-warning-600">*</span>
                  </label>
                  <p className="text-xs text-neutral-600 dark:text-neutral-400 mb-3">
                    Clearly explain what additional information or clarification is needed for KYC verification.
                  </p>
                  <textarea
                    value={requestInfoMessage}
                    onChange={(e) => setRequestInfoMessage(e.target.value)}
                    placeholder="Example: Please upload a clearer photo of your national ID card. The current image is too blurry to verify the details."
                    rows={6}
                    className="w-full px-4 py-3 bg-white dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-600 rounded-lg text-neutral-900 dark:text-white placeholder-neutral-400 dark:placeholder-neutral-500 focus:outline-none focus:ring-2 focus:ring-warning-500 dark:focus:ring-warning-400 resize-none"
                  />
                  <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-2">
                    {requestInfoMessage.length} / 500 characters
                  </p>
                </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-warning-700 dark:text-warning-400 mt-1">
                        The member will be notified to resubmit their KYC with the requested information. Their status will change to "Pending" until they respond.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="px-6 py-4 border-t border-neutral-200 dark:border-neutral-800 flex justify-end gap-3">
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-warning-600 hover:bg-warning-700 text-white font-semibold rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
