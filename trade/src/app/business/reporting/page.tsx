...
export default function BusinessReportingPage() {
  ...
  const fetchBusinessData = async () => {
    ...
    try {
      ...
      if (userApplication) {
        ...
        setContractType(userApplication.data.contractType || null);
  ...
  const handleContractSpecificChange = (field: string, value: any) => {
    if (!contractType) return;

    const detailsKey = `${contractType}Details` as keyof RevenueReport;
    
    setFormData((prev) => ({
      ...prev,
      [detailsKey]: {
        ...(prev[detailsKey] as any || {}),
        [field]: value,
      },
    }));
  };
            ...
            {contractType && (
              <div className="mt-6">
                {contractType === "murabaha" && (
                  <MurabahaReportFields
                    details={formData.murabahaDetails || {}}
                    onChange={handleContractSpecificChange}
                    errors={errors}
                  />
                )}
                {contractType === "mudaraba" && (
                  <MudarabaReportFields
                    details={formData.mudarabaDetails || {}}
                    onChange={handleContractSpecificChange}
                    errors={errors}
                  />
                )}
                {contractType === "musharaka" && (
                  <MusharakahReportFields
                    details={formData.musharakahDetails || {}}
                    onChange={handleContractSpecificChange}
                    errors={errors}
                  />
                )}
                {contractType === "ijara" && (
                  <IjaraReportFields
                    details={formData.ijaraDetails || {}}
                    onChange={handleContractSpecificChange}
                    errors={errors}
                  />
                )}
                {(contractType === "salam" || contractType === "istisna") && (
                  <SalamReportFields
                    details={formData.salamDetails || {}}
                    onChange={handleContractSpecificChange}
                    errors={errors}
                  />
                )}
