  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      unsubscribe = onAuthStateChange(async (user) => {
        ...
        try {
          console.log("=== SAVING TO JUNO DATASTORE ===");
          console.log("User key:", user.key);
          console.log("Collection: individual_investor_profiles");
          ...
          let existingProfile;
          try {
            existingProfile = await getDoc({
              collection: "individual_investor_profiles",
              key: user.key
            });
            console.log("Found existing profile, will update it");
          } catch (err) {
            console.log("No existing profile found, will create new one");
          }
          
          // Save directly to Juno datastore
          const savedDoc = await setDoc({
            collection: "individual_investor_profiles",
            doc: {
              key: user.key,
              data: profileData,
              // Include version if updating existing document
              ...(existingProfile ? { version: existingProfile.version } : {})
            }
          });
          
          console.log("✅ Profile saved to Juno successfully!");
          console.log("Saved document:", savedDoc);
          
          // ALSO save to localStorage for backwards compatibility with admin dashboard
          ...
          console.log("✅ Also saved to localStorage for admin dashboard");
          ...
          console.error("❌ Error saving profile to Juno:", err);
          console.error("Error details:", JSON.stringify(err, null, 2));
          setErrors({ submit: `Failed to submit application: ${err}. Please try again.` });
          setLoading(false);
        }
      });
    } catch (error) {
