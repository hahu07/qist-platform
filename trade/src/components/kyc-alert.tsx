...
export function KYCAlert({ kycStatus, profile }: KYCAlertProps) {
  ...
  const canResubmit = profile?.rejectionAllowsResubmit !== false;
  ...
  const alertConfig = {
    ...
    rejected: {
      title: canResubmit ? "KYC Verification Rejected" : "KYC Verification Permanently Rejected",
      message: canResubmit 
        ? "Unfortunately, your KYC documents were not approved. Please review the feedback and resubmit with corrected documents."
        : "Your KYC application has been permanently rejected. Please contact support for assistance.",
      buttonText: canResubmit ? "Resubmit Documents" : "Contact Support",
      buttonHref: canResubmit ? "/member/kyc" : "/support",
    ...
    <div className={`${config.bgColor} border-l-4 ${config.borderColor} rounded-lg p-6 mb-6 shadow-sm`}>
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          {/* Show rejection details if KYC was rejected */}
          {kycStatus === "rejected" && (rejectionReason || adminMessage) && (
            <div className="mb-4 p-4 bg-white dark:bg-neutral-800 rounded-lg border border-error-200 dark:border-error-800">
              <p className="text-sm font-semibold text-error-900 dark:text-error-100 mb-2">
                Rejection Details:
              </p>
              {rejectionReason && (
              ...
              {adminMessage && adminMessage !== rejectionReason && (
                <p className="text-sm text-error-800 dark:text-error-200">
                  <span className="font-medium">Additional Note:</span> {adminMessage}
                </p>
              )}
              {!canResubmit && (
                <p className="text-xs text-error-700 dark:text-error-300 mt-3 p-2 bg-error-100 dark:bg-error-900/30 rounded">
                  ⚠️ This is a permanent rejection. Please contact our support team for further assistance.
                </p>
              )}
            </div>
          )}
          
          {/* Show admin message if there's a pending request for more info */}
          {kycStatus === "pending" && adminMessage && (
            <div className="mb-4 p-4 bg-white dark:bg-neutral-800 rounded-lg border border-warning-200 dark:border-warning-800">
              <p className="text-sm font-semibold text-warning-900 dark:text-warning-100 mb-2">
